...
def ReadFile(File: str = None, Model: str = None, Test: str = None):
    ...
    - In vivo:
        - Length and torque scaling factors
    ...
    if Model == 'Single Fibre':
        ...
        Data['Normalized Length'] = Data['Length'].div(FibreLength)
...
def pCaAnalysis(Data: pd.DataFrame = None, CSA: float = None, Graph: bool = False) -> tuple[pd.DataFrame, float, float]:
    ...
    SubsetData = Data[100000:300000]
    
    # Find highest rolling 500 ms window in force
    WindowLength = 5000
    GraphWindow = int(WindowLength / 2)
    RollingMean = pd.DataFrame(SubsetData['Force'].rolling(window = WindowLength, center = True).mean())
    PeakForce = SubsetData['Force'].rolling(window = WindowLength, center = True).mean().max()
    SpecificForce = PeakForce / CSA

    Peakindex = RollingMean.index[RollingMean['Force'] == PeakForce][0]

    if Graph == True:
        plt.plot(Data['Time'], Data['Force'], color = Colors.Black)
        plt.plot(Data['Time'][Peakindex - GraphWindow: Peakindex + GraphWindow], Data['Force'][Peakindex - GraphWindow: Peakindex + GraphWindow], color = Colors.Firebrick)
        plt.show()

    return Data, PeakForce, SpecificForce

def ktrAnalysis(Data: pd.DataFrame = None, StiffnessTimeSeconds: float = 0.9, SampleRate: int = Constants.SampleRate, Graph: bool = False) -> tuple[float, float, float, pd.Series, pd.Series, float]:
    """
    Model ktr \n
    Returns:
    ...
    StiffnessTime = StiffnessTimeSeconds * SampleRate
    ...
    return Stiffness, ktr, GoodnessFit, Xmodel, Ymodel, ktrForce
...
def PowerAnalysis(Data: pd.DataFrame = None, PowerLoads: list = None, CSA: float = None, FibreLength: float = None, Graph: bool = None) -> tuple[float, dict]:
    ...
    if Graph == True:
        ...
        AllColors = [Colors.DeepBlue, Colors.Firebrick, Colors.Sienna, Colors.SeaGreen]
        for idx, Clamp in enumerate(Constants.ForceClampTimes):
            ...
            LineColor = AllColors[idx]

            Force_Inset.plot(Data['Time'][Calculation_Start:Calculation_End], Data['Force'][Calculation_Start:Calculation_End], color = LineColor, label = 'Calculations')
            Length_Inset.plot(Data['Time'][Calculation_Start:Calculation_End], Data['Length'][Calculation_Start:Calculation_End], color = LineColor, label = 'Calculations')
...
def rFEAnalysis(Data: pd.DataFrame = None, StiffnessTimeSeconds: int = 65, SampleRate: int = Constants.SampleRate, Graph: bool = False) -> tuple[float, float, float]:
    """
    Returns:
     - Orginal dataframe (with baseline force subtracted from force column)
    ...
    PeakForce = Data['Force'][Force_Window].mean()

    PassiveForce = Data['Force'][Passive_Window].mean()

    if Graph == True:

        pass

    return Data, PeakForce, PassiveForce, Stiffness
...
def Torque_Frequency(Data: pd.DataFrame = None, Timepoint: str = None, Subject: str = None, Hz: int = None, Graph: bool = False) -> float:
    ...
    if Hz >= 80:
        WindowLength = 500
        GraphWindow = int(WindowLength / 2)
        RollingMean = pd.DataFrame(Data['Filtered Torque'][StimIndex:].rolling(window = WindowLength, center = True).mean())
        PeakTorque = Data['Filtered Torque'][StimIndex:].rolling(window = WindowLength, center = True).mean().max()
        Peakindex = RollingMean.index[RollingMean['Filtered Torque'] == PeakTorque][0]

        if Graph == True:
            plt.plot(Data['Time'], Data['Filtered Torque'], color = Colors.Black)
            plt.plot(Data['Time'][Peakindex - GraphWindow : Peakindex + GraphWindow], Data['Filtered Torque'][Peakindex - GraphWindow : Peakindex + GraphWindow], color = Colors.Firebrick)
            plt.show()
    
    else:
        # For non-tetanic contractions, peak torque is highest single point during 500 ms window where stims are delivered
        PeakTorque = Data['Filtered Torque'][StimIndex : StimIndex + 5000].max()
...
def Torque_Velocity(Data: pd.DataFrame = None, Timepoint: str = None, ISO_Percent: str = None, Subject: str = None) -> tuple[float, float, float]:
...
def RTD_Analysis(Data: pd.DataFrame = None, PeakTorque: float = None):
    ...
    plt.plot(Data['Time'], Data['Filtered Torque'], color = Colors.Black)
    plt.plot(Data['Time'][ContractionStart:ContractionEnd], Data['Filtered Torque'][ContractionStart:ContractionEnd], color = Colors.Firebrick)
...
def PLFFD(Data: pd.DataFrame = None, File: str = None, RecoveryTimepoint: str = None, Subject: str = None, Timepoint: str = None, Graph: bool = False) -> tuple[float, float, float, float, float]:
    ...
    if Graph == True:
        ...
        PLFFD_Fig.plot(Data['Time'], Data['Filtered Torque'], color = Colors.Charcoal)
...
def Means_and_SEM(Timepoint: str, Test: str, Results: pd.DataFrame) -> tuple[dict, dict]:
    ...
    if Test == 'Torque-Frequency':
        ...
        for Percent in Constants.ISO_Percents:
            for column in Results.columns:
